...
from data_control_service.domain.models import RouteDecision
from data_control_service.domain.policies import RESOURCE_REGISTRY, ResourceRegistration
...
class RoutingService:
    ...
    def get_registration(self, request: DataRequest) -> ResourceRegistration:
        key = (request.resource.target, request.resource.type, request.resource.name)
        registration = RESOURCE_REGISTRY.get(key)
        if registration is None:
            raise DataControlError("RESOURCE_TYPE_UNKNOWN")
        return registration

    def route(self, request: DataRequest, registration: ResourceRegistration) -> RouteDecision:
        ...
        return RouteDecision(
            ...
            logical_resource=f"{registration.resource_type}:{registration.name}",
